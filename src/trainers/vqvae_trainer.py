...
class VQVAETrainer:
    def __init__(self, args):
        ...
        if checkpoint_path.exists():
            ...
            print(f"Resuming training using checkpoint {checkpoint_path} at epoch {self.start_epoch}")
        else:
            self.start_epoch = 0
            self.best_loss = 1000
            self.global_step = 0
        ...
        if self.ddp:
            self.model = DistributedDataParallel(
                self.model, device_ids=[self.device], find_unused_parameters=False, broadcast_buffers=False,
            )
            self.discriminator = DistributedDataParallel(
                self.discriminator, device_ids=[self.device], find_unused_parameters=False, broadcast_buffers=False,
            )
    ...
    def train_epoch(self, epoch):
        ...
        for step, batch in progress_bar:
            ...
            adversarial_loss = self.adv_loss(logits_fake, target_is_real=True, for_discriminator=False)
            ...
            self.logger_train.add_scalar(tag="l1_loss", scalar_value=recons_loss.item(), global_step=self.global_step)
            self.logger_train.add_scalar(tag="perceptual_loss", scalar_value=perceptual_loss.item(), global_step=self.global_step)
            self.logger_train.add_scalar(tag="generator_loss", scalar_value=total_generator_loss.item(), global_step=self.global_step)
            self.logger_train.add_scalar(tag="discriminator_loss", scalar_value=discriminator_loss.item(), global_step=self.global_step)
            if self.quick_test:
                break
    ...
    def val_epoch(self, epoch):
        with torch.no_grad():
            ...
            for step, batch in progress_bar:
                ...
                adversarial_loss = self.adv_loss(logits_fake, target_is_real=True, for_discriminator=False)
                total_generator_loss = (
                    recons_loss + quantization_loss + self.perceptual_weight * perceptual_loss + jukebox_loss + self.adv_weight * adversarial_loss
                )
                self.logger_val.add_scalar(tag="l1_loss", scalar_value=recons_loss.item(), global_step=global_val_step)
                self.logger_val.add_scalar(tag="generator_loss", scalar_value=total_generator_loss.item(), global_step=global_val_step)
                ...
                if step == 0:
                    ...
                    for i in range(2):
                        ...
                        if i == 0:
                            plt.title("Recon")
                    plt.show()
                    self.logger_val.add_figure(tag="recons", figure=fig, global_step=self.global_step)
